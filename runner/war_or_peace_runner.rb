require './lib/card'
require './lib/deck'
require './lib/player'
require './lib/turn'
require './lib/start'

card1 = Card.new(:spade, 'King', 13)
card2 = Card.new(:spade, 'Queen', 12)
card3 = Card.new(:spade, 'Jack', 11)
card4 = Card.new(:spade, '10', 10)
card5 = Card.new(:spade, '9', 9)
card6 = Card.new(:spade, '8', 8)
card7 = Card.new(:spade, '7', 7)
card8 = Card.new(:spade, '6', 6)
card9 = Card.new(:spade, '5', 5)
card10 = Card.new(:spade, '4', 4)
card11 = Card.new(:spade, '3', 3)
card12 = Card.new(:spade, '2', 2)
card13 = Card.new(:spade, 'Ace', 14)
card14 = Card.new(:club, 'King',13)
card15 = Card.new(:club, 'Queen', 12)
card16 = Card.new(:club, 'Jack', 11)
card17 = Card.new(:club, '10', 10)
card18 = Card.new(:club, '9', 9)
card19 = Card.new(:club, '8', 8)
card20 = Card.new(:club, '7', 7)
card21 = Card.new(:club, '6', 6)
card22 = Card.new(:club, '5', 5)
card23 = Card.new(:club, '4', 4)
card24 = Card.new(:club, '3', 3)
card25 = Card.new(:club, '2', 2)
card26 = Card.new(:club, 'Ace', 14)
card27 = Card.new(:diamond, 'King', 13)
card28 = Card.new(:diamond, 'Queen', 12)
card29 = Card.new(:diamond, 'Jack', 11)
card30 = Card.new(:diamond, '10', 10)
card31 = Card.new(:diamond, '9', 9)
card32 = Card.new(:diamond, '8', 8)
card33 = Card.new(:diamond, '7', 7)
card34 = Card.new(:diamond, '6', 6)
card35 = Card.new(:diamond, '5', 5)
card36 = Card.new(:diamond, '4', 4)
card37 = Card.new(:diamond, '3', 3)
card38 = Card.new(:diamond, '2', 2)
card39 = Card.new(:diamond, 'Ace', 14)
card40 = Card.new(:heart, 'King', 13)
card41 = Card.new(:heart, 'Queen', 12)
card42 = Card.new(:heart, 'Jack', 11)
card43 = Card.new(:heart, '10', 10)
card44 = Card.new(:heart, '9', 9)
card45 = Card.new(:heart, '8', 8)
card46 = Card.new(:heart, '7', 7)
card47 = Card.new(:heart, '6', 6)
card48 = Card.new(:heart, '5', 5)
card49 = Card.new(:heart, '4', 4)
card50 = Card.new(:heart, '3', 3)
card51 = Card.new(:heart, '2', 2)
card52 = Card.new(:heart, 'Ace', 14)
full_deck = [card1, card2, card3, card4, card5, card6, card7, card8, card9, card10, card11, card12, card13, card14, card15, card16, card17, card18, card19, card20, card21, card22, card23, card24, card25, card26, card27, card28, card29, card30, card31, card32, card33, card34, card35, card36, card37, card38, card39, card40, card41, card42, card43, card44, card45, card46, card47, card48, card49, card50, card51, card52]

full_deck.shuffle!
p1_deck = full_deck.slice(0..25)
p2_deck = full_deck.slice(26..51)

@player1 = Player.new('Megan', p1_deck)
@player2 = Player.new('Aurora', p2_deck)

@start = Start.new(@turn)
@turn = Turn.new(@player1, @player2)

puts "Welcome to War! (or Peace) This game will be played with 52 cards.
The players today are Megan and Aurora.
Type 'GO' to start the game!
------------------------------------------------------------------"

puts "When you're ready type 'GO' to start the game"

user_input = gets.chomp

if user_input == "GO"
  @start.start

else
  p "Sorry you don't want to play, maybe next time!"
end
